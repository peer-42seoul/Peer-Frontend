...
const TagAutoComplete = ({ list, datas, setData, placeholder }: { list: string[], datas: string[], setData: any, placeholder?: string }) => {
    /* 태그를 추가합니다 */
    const handleInput = (
    ...
    const handleDelete = (index: number) => {
        setData((chips: string[]) => chips.filter((chip, cIndex) => cIndex !== index));
    }
        ...
        <>
            <Autocomplete
                ...
                renderInput={(params) => (
                    <TextField
                        {...params}
                        placeholder={placeholder ?? "프레임워크 또는 개발언어를 입력해주세요."}
                    />
                )}
            />
