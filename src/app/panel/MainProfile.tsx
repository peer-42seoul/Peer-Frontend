...
const MainProfile = () => {
    ...
    const { data } = useSWR<IUserProfile>(
        isLogin ?
            [`${process.env.NEXT_PUBLIC_API_URL}/api/v1/profile`, axiosWithAuth] : undefined,
        ([url, axiosWithAuth]) =>
            getFetcherWithInstance(url, axiosWithAuth as AxiosInstance),
    )

    return (
        <Stack height="150px" alignItems="center" justifyContent={"center"} border="1px solid black">
            <Avatar alt="avatar" src={data?.profileImageUrl} sx={{ width: 56, height: 56 }} />
            {
                isLogin ? <><Typography>{data?.nickname}</Typography>
